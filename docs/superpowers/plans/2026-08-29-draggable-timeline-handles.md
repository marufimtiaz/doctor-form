# Interactive Draggable Timeline Handles Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement interactive draggable timeline handles for Start and End shift times, along with a smart `+ Add Handle` button (max 2 handle pairs per group) that discovers the farthest free 30-minute time slot.

**Architecture:** Component-level drag handlers and smart space search algorithm in `frontend/src/components/SlotEditor.tsx`, maintaining full compatibility with `surveySchema` and `toBackendSlots`.

**Tech Stack:** React, React Hook Form, Lucide React, Tailwind CSS / Shadcn UI components.

## Global Constraints

- Timeline range: 08:00 AM (480 mins) to 11:00 PM (1380 mins) with 30-minute steps
- Maximum 2 handle pairs (2 shift ranges) per slot group
- Smart empty space search for `+ Add Handle` button
- Zero backend API or DB schema changes

---

### Task 1: Update `SlotEditor.tsx` with Interactive Drag Handles & Smart Space Search
...
- Renders:
  - Timeline track with draggable left (`start_time`) and right (`end_time`) knobs for each active range.
  - `+ Add Handle` button to the right of the timeline bar (disabled if 2 handle pairs already exist or no free 30-min slot is available).
  - Smart space discovery algorithm finding the free 30-min slot farthest from existing shifts.

- [ ] **Step 1: Implement Draggable Handles & Smart Space Search in `SlotEditor.tsx`**
...
function findFarthestFreeSlot(ranges: Array<{ start_time: string; end_time: string }>): { start: string; end: string } | null {
  // Collect 30-min slots from 08:00 (16 slots of 30m = 8h to 23h = 30 intervals)
  ...
  // Find occupied indices to calculate distance
  const occIndices = isOccupied.map((occ, idx) => (occ ? idx : -1)).filter((idx) => idx !== -1);

  if (occIndices.length === 0) {
    // Default evening slot if completely empty
    return { start: "17:00", end: "20:00" };
  }

  // Calculate distance of each free index to closest occupied index
  let maxDist = -1;
...
- [ ] **Step 2: Verification with TypeScript & Vitest**
...
git commit -m "feat(ui): implement interactive draggable timeline handles and smart farthest free slot discovery"
```
